...
VoteModule.prototype.onStart = function(data)
{
    if (data.sender != global.misaka.config.obj.master)
        return ;
    if (this._started)
        return this.getString(this.C.votestart.vote_already_started, data.sender);
    this._votes = {};
    ...
    if (!this._options.length)
        return this.getString(this.C.votestart.no_options, data.sender);
    this._started = true;
    return this.getString(this.C.votestart.message, data.sender).replace(/%options/g, this._options.join(", "));
};
...
VoteModule.prototype.onInfo = function(data)
{
    ...
    if (!this._started)
        return this.getString(this.C.voteinfo.no_vote_started, data.sender);
    return this.getString(this.C.voteinfo.message, data.sender).replace(/%options/g, this._options.join(", "));
};
...
VoteModule.prototype.onState = function(data)
{
    ...
    if (!this._started)
        return this.getString(this.C.votestate.no_vote_started, data.sender);
    var votes = [];
    ...
    if (!state.length)
        return this.getString(this.C.votestate.no_vote_yet, data.sender);
    return this.getString(this.C.votestate.message, data.sender).replace(/%state/g, state.join(", "));
};
...
VoteModule.prototype.onEnd = function(data)
{
    if (data.sender != global.misaka.config.obj.master)
        return ;
    if (!this._started)
        return this.getString(this.C.voteend.no_vote_started, data.sender);
    this._started = false;
    var votes = {};
    for (var n in this._votes)
        votes[this._votes[n]] = (votes[this._votes[n]] || 0) + 1;
    ...
    if (!winners.length)
        return this.getString(this.C.voteend.nobody_voted, data.sender);
    if (winners.length == 1)
        return this.getString(this.C.voteend.message, data.sender).replace(/%result/g, winners[0]);
    else if (winners.length > 1)
        return this.getString(this.C.voteend.message_draw, data.sender).replace(/%result/g, winners.join(", "));
};
...
VoteModule.prototype.onMessage = function(data)
{
    ...
    if (this._started && data.msg.trim()[0] == '#' && data.username != global.misaka.config.obj.username)
    {
        ...
        if (optionId === undefined)
            return this.send(this.getString(this.C.invalid_option, data.username).replace(/%option/g, option));
        if (this._votes[data.username] !== undefined && this._votes[data.username] != optionId && this.C.user_changed_vote && this.C.user_changed_vote.length)
            this.send(this.getString(this.C.user_changed_vote, data.username).replace(/%option/g, option));
...
VoteModule.prototype.send = function(message)
{
